...
writePerm <- function(df, suffix, excl = NULL, qc = FALSE){
  ...
  if(qc==TRUE){
    write.csv(df[, qckeepVars, with=FALSE], paste0("../data/QC_", outname, ".csv"), row.names=FALSE)
    message("Writing ", paste0("../data/QC_", outname, ".csv"))
  } else {
    assign(outname, df, topenv())
  }
}
...
saveObj <- function(inobj, suffix, prefix = pkgName){
  outname <- paste0(prefix, "_", suffix)
  assign(outname, get(inobj), envir = topenv())
