---
title: Uberla 465 BAMA Processing
shorttitle: Uberla 465 BAMA
from:
- name: Paul Obrecht
  email: pobrecht@fredhutch.org
summary: BAMA processing for the Uberla 465 study
vignette: >
  %\VignetteIndexEntry{Uberla 465 BAMA Processing}
...
This report documents the Uberla 465 BAMA data processing.
...
# background <- copy(bama)[specrole=="Background" & averaged=='false', .(bkgd1 = mean(fi, na.rm=TRUE)), .(filename, antigen)]
...
# bama[, bkgd := fi - fi_bkgd]
# bama <- merge(bama, background, by = c("filename", "antigen"), all.x=TRUE)[, antigen := ifelse(tolower(antigen) %in% blanklist[["gp70"]], "gp70 control", antigen)]
# test <- bama[, .(num_wells = nrow(.SD)), .(readerserial, filename, antigen, dilution, bkgd1)]
```
...
kableit(bama[, .N, n_repl], caption="Summary of number of replicates in averaged data")
```
...
runrx <- ifelse(file.exists("../inst/extdata/rx.csv"), TRUE, FALSE) # merge in rx data and csv is present?
runarp <- ifelse(file.exists("../inst/extdata/arp.csv"), TRUE, FALSE) # merge in arp data and csv is present?
...
arp <- fread("../inst/extdata/arp.csv")
setkey(arp, antigen)
setkey(bama, antigen)
...
ser[, # use only original 
    pos_threshold := as.integer(max(100,
                                    floor(quantile(delta[sourcefile == bama_qfilenames[1] &
                                                         visitno==baseline_visit &
                                                         tolower(filter_reason) == "not filtered"],
                                                   probs = 0.95, na.rm=TRUE)))),
    by = .(isotype, antigen)]
kableit(ser[sourcefile == bama_qfilenames[1], # use only original 
            .N, .(isotype, antigen, pos_threshold)], caption = "Deritived positivity cutoffs (95th percentile of unfiltered baseline deltas")
```

```{r 'auc', eval=runtitrations, echo=runtitrations}
cat("The AUTC (area under the titration curve) is computed. For Uberla 465, we use the November 14 file for AUTC calculation.\n")
byvars <- c("spec_primary", "isotype", "ptid", "visitno", "antigen")
setkeyv(ser, c(byvars, "dilution"))

excl <- ser[dilution == binding_dil & filter_reason != "Not Filtered"]
excl <- interaction(excl$ptid, excl$antigen)
auc <- ser[sourcefile %in% bama_qfilenames[2:3] & !interaction(ptid, antigen) %in% excl,
...
# for Uberla 465 reruns
kableit(dcast.data.table(ser[dilution==binding_dil], filter_reason + visitno + sourcefile ~ response, value.var="response", fun.aggregate = length), caption="Response call by source file, visit, and filter_reason")

# compare at ptid/antigen level
j <- dcast(ser[dilution==binding_dil], ptid + antigen + visitno ~ sourcefile, value.var = "response")
j <- j[visitno > -2 & antigen %in% unique(ser[sourcefile == bama_qfilenames[2]]$antigen)]
j <- j[`CAVDUberla 465_IgG_Serum_20170905.txt` != `CAVDUberla 465_IgG_Serum_Titrations_20180228.txt`]
kableit(j, caption="PTID/antigen combinations that change response call from first to second assay")

# set response call to missing in 2nd assay
ser[sourcefile == bama_qfilenames[2], response := NA]
```
...
cat("## Titration Data\nIf titrations were run, we create a separate titrations dataset containing all dilutions, as well as a serum_bama dataset containing just the direct binding dilution. For Uberla 465, the screening dilution dataset contains just those records submitted in the September 5, 2017 qdata file. The titrations dataset contains just those records submitted in the November 14, 2017 qdata file.")
dil <- ser[sourcefile %in% bama_qfilenames[2:3]]
ser <- ser[dilution == binding_dil & sourcefile %in% bama_qfilenames[c(1,3)]]
...
if(!exists("dil")){dil <- ser} # we create dil to use below, but we don't create a second duplicate dataset called "titrations"
```
...
```{r 'writerx', eval=exists("rx"), echo=exists("rx")}
cat("## Treatment assignment data\nOutput rx.rda of treatment assignment information.")
setkey(rx, group, ptid)
saveObj("rx", "rx")
```

```{r 'writearp', eval=exists("rx"), echo=exists("rx")}
cat("## ARP data\nOutput arp.rda of ARP information.")
setkey(arp, antigen)
saveObj("arp", "arp")
```
